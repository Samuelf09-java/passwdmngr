...
static void on_view_delete(EntryViewBox *view_box, MainWindow *self) {
    ...
    GtkAlertDialog *dialog = gtk_alert_dialog_new("Are you sure you want to delete this entry?\n"
...
static void on_edit_save(EntryEditBox *edit_box, MainWindow *self) {
    ...
    if (!strlen(new_username))
        util_warn_d("Field 'username' is missing, continuing");
    char *password = strdup(gtk_editable_get_text(GTK_EDITABLE(edit_box->password_entry)));
    if (!strlen(password))
        util_warn_d("Field 'password' is missing, continuing");
...
static void on_entry_selected(GtkListBox *box, GtkListBoxRow *row, MainWindow *self) {
    ...
    if (strlen(notes) == 0)
        gtk_label_set_text(GTK_LABEL(view_box->notes_title), "");
...
static void on_cancel_rename_entry_clicked(GtkButton *button, gpointer user_data) {
    ...
    GtkAlertDialog *dialog =
        gtk_alert_dialog_new("Duplicate entry '%s'\n"
                             "Choose one option:",
...
static void on_dupe_import_choice(GObject *source, GAsyncResult *result, gpointer user_data) {
    ...
    if (source) { // rename callback passes NULL, NULL, context, so this should only be run if it is called as an actual
                  // callback
    ...
    for (int i = context->current_index; i < context->mode_data->num_import_entries; i++) {
        ...
        if (dupe_service) {
            GtkAlertDialog *dialog = gtk_alert_dialog_new("Duplicate entry '%s'\n"
                                                          "Choose one option:",
...
static void on_confirm_choose_entry_imp_clicked(GtkButton *button, gpointer user_data) {
    ...
    // select import mode (replace current data (DANGEROUS, will make backup), append to current data (w/overwrite
    // dupes), or append (prompt to overwrite))
...
static void on_confirm_importpasswd_clicked(GtkButton *button, gpointer user_data) {
    ...
    data->num_import_entries =
        storage_read_vault_with_key(data->vault_path, key, &(data->import_entries), &(data->header));
...
void register_actions(MainWindow *self) {
    const GActionEntry entries[] = {
        {"export", export_cb, NULL, NULL, NULL, {0}},       {"import", import_cb, NULL, NULL, NULL, {0}},
        {"openlog", openlog_cb, NULL, NULL, NULL, {0}},     {"clearlog", clearlog_cb, NULL, NULL, NULL, {0}},
        {"logout", logout_cb, NULL, NULL, NULL, {0}},       {"changepasswd", changepasswd_cb, NULL, NULL, NULL, {0}},
...
static void main_window_class_init(MainWindowClass *klass) {
    ...
    gtk_widget_class_set_template_from_resource(widget_class, "/com/samuelf09/passwdmngr/main_window.ui");
...
static void main_window_init(MainWindow *self) {
    ...
    GtkBuilder *builder = gtk_builder_new_from_resource("/com/samuelf09/passwdmngr/main_menu.ui");
